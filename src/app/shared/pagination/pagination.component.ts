...
export class PaginationComponent implements OnInit, OnDestroy {
  ...
  updatePageList() {
    this.currentPageSize = Math.ceil(this.list.length / this.numOfItemOnPage) < this.pageSize
  ...
  updateDisplayList(current: number) {
    const newList: User[] = this.list.slice((current - 1) * this.numOfItemOnPage, current * this.numOfItemOnPage);
